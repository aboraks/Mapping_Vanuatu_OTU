...
rm(list=ls())
# First try and pick out some important species that would be worthwile 
...
for (HUGE in 1:length(Trnsx)) {
  

# subset to a sample type
Ssbst <- subset_samples(vst_physeq, Sample_Type == "Selaginella" & Transect== Trnsx[HUGE])
...
Gsbst <- subset_samples(vst_physeq, Sample_Type == "Ground" & Transect== Trnsx[HUGE])
...
Usbst <- subset_samples(vst_physeq, Sample_Type == "Understory" & Transect== Trnsx[HUGE])
...
Gfungi <- Gotu_even[,(colnames(Gotu_even) %in% colnames(Uotu_even))]
Ufungi <- Uotu_even[,(colnames(Uotu_even) %in% colnames(Gfungi))]
Sfungi <- Sotu_even[,(colnames(Sotu_even) %in% colnames(Gfungi))]

Ufungi <- Ufungi[,(colnames(Ufungi) %in% colnames(Sfungi))]
Gfungi <- Gfungi[,(colnames(Gfungi) %in% colnames(Sfungi))]

otu_names <- colnames(Gfungi)
...
print(colnames(Gfungi))


#######################
# Ground
#######################
# Subset the OTU dataset to only the names found in the list currated above 
Gfungi <- t(otu_table(Gsbst))
Gfungi <- Gfungi[,(colnames(Gfungi) %in% otu_names)]
# make sure everything is in order
Gfungi <- Gfungi[,order(colnames(Gfungi))]

# some OTUs are now at a 0 value due to subsetting- remove any OTU less than 0
Gfungi[Gfungi < 0.0] <- 0.0
# any sample id that were removed in the OTU culling process should also be removed from the Sample ID list 
Gsample <- sample_data(Gsbst)
Gsample <- Gsample[(rownames(Gsample) %in% (rownames(Gfungi))),]

#######################
# Understory
#######################
# Subset the OTU dataset to only the names found in the list currated above 
Ufungi <- t(otu_table(Usbst))
Ufungi <- Ufungi[,(colnames(Ufungi) %in% otu_names)]
# make sure everything is in order
Ufungi <- Ufungi[,order(colnames(Ufungi))]

# some OTUs are now at a 0 value due to subsetting- remove any OTU less than 0
Ufungi[Ufungi < 0.0] <- 0.0
# any sample id that were removed in the OTU culling process should also be removed from the Sample ID list 
Usample <- sample_data(Usbst)
Usample <- Usample[(rownames(Usample) %in% (rownames(Ufungi))),]

#######################
# Selaginella
#######################
# Subset the OTU dataset to only the names found in the list currated above 
Sfungi <- t(otu_table(Ssbst))
Sfungi <- Sfungi[,(colnames(Sfungi) %in% otu_names)]
# make sure everything is in order
Sfungi <- Sfungi[,order(colnames(Sfungi))]

# some OTUs are now at a 0 value due to subsetting- remove any OTU less than 0
Sfungi[Sfungi < 0.0] <- 0.0
# any sample id that were removed in the OTU culling process should also be removed from the Sample ID list 
Ssample <- sample_data(Ssbst)
Ssample <- Ssample[(rownames(Ssample) %in% (rownames(Sfungi))),]



#call tha taxomony table 
fungID <- tax_table(Gsbst)


#soil_colour <- data.frame("colour" = "#663300")
#selaginella_colour <- data.frame("colour" = "#CC9900")
#understory_colour <- data.frame("colour" = "#003300")
#tree_colour <- data.frame("colour" = "#990000")

# I think this is where you want to do a loop 
# 

#select(Gsample,Sample_Type,Transect_Short,Transect_Long)
#Gfungi[,1]
for (BIG in 1:length(colnames(Gfungi))) {
# make a table of sample location and abundence of fungal isloate [,#]
  
Gtest <- cbind(select(Gsample,Sample_Type,Transect_Short,Transect_Long),Gfungi[,BIG])#,soil_colour)

Gtest <- rbind(Gtest,cbind(select(Usample,Sample_Type,Transect_Short,Transect_Long),Ufungi[,BIG]))#,understory_colour))
Gtest <- rbind(Gtest,cbind(select(Ssample,Sample_Type,Transect_Short,Transect_Long),Sfungi[,BIG]))#,selaginella_colour))



Gtest$Transect_Short <- as.numeric.factor(Gtest$Transect_Short) # house keeping.. Factor->numeric
Gtest$Transect_Long <- as.numeric.factor(Gtest$Transect_Long) # house keeping.. Factor->numeric

taxID <- paste(fungID[colnames(Gfungi[,BIG]),2],fungID[colnames(Gfungi[,BIG]),3],"\n",fungID[colnames(Gfungi[,BIG]),4],fungID[colnames(Gfungi[,BIG]),5],"\n",fungID[colnames(Gfungi[,BIG]),6],fungID[colnames(Gfungi[,BIG]),7])


ggplot(data = Gtest)+
geom_point(aes(x=Gtest$Transect_Short, y=Gtest$Transect_Long, color = Sample_Type), data=Gtest, size=Gtest[,4], shape=0, show.legend = FALSE)+
  labs(title = paste(colnames(Gfungi[,BIG])),
       subtitle = paste(taxID))+
  theme_minimal()+
  theme(axis.title.x=element_blank(),axis.title.y = element_blank())


ggsave(plot = last_plot(), 
        filename = file.path("/Users/fungi2/Google Drive/Projects/Vanuatu/Vanuatu Molecular/New_Analysis/R/Tran.map.sp2", #where to save it
                                                 paste("Transect",Trnsx[HUGE], colnames(Gfungi[,BIG]),".jpg")), #name
        device = NULL,
        scale = 1, width = 4, height = 7, units = "in", 
...
for (QQQ in 2:2) { # use this loop start to generate a series of tables (see line ~ 632: write.table(sig.results.undr, file = paste("./Mapping.single.sp/Mantel.0.05/sig.sa.undr.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)


ground <- data.frame(matrix(ncol=7,nrow=0))
understory <- data.frame(matrix(ncol=7,nrow=0))
# QQQ=5
#for (QQQ in 12:12) {  # this is the minimum number of times an OTU needs to be found in each transect - for a table use:             for (QQQ in 1:36) {
...
# for exampl: "Select only fungi that are found in the Understory a minimum of 5 times, and are also found in soil less than 2 occurences)
...
write.table(ground.results, file = paste("./Mapping.single.sp/Mantel.0.05/Patch.occupancy/special.grnd.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
write.table(under.results, file = paste("./Mapping.single.sp/Mantel.0.05/Patch.occupancy/special.undr.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
...
# to get things kicked off - I'll choose the minimum number of observations/transect to be 17 
# with QQQ=17 we should have ONE otu that can be found in 3 transects for each Understory and Ground (see slack (or run loop)) 
...
# now you have a list of OTU IDs and where to find them 
undr
grnd
...
# 3 Check for spatial autocorrelation and Make variograms for each OTU+transect - combine individual OTU from multiple transects to form a composite variogram 
    # 3.a - "Combining data from multiple years or areas to improve variogram estimation"# pare sample_data table down by only the remaining sample ids in the OTU table 
            # isolate an OTU identified above
            # grab unique transect ids from sample-data table 
            # start loop investigating each transect separatly
            # for transect i make variogram 
            # inspect 
            # figure out how to combine variograms 
# GROUND #
...
####################################
# make a figure describing the above loop results
...
write.table(sig.results.undr, file = paste("./Mapping.single.sp/Mantel.0.05/sig.sa.undr.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
write.table(sig.results.grnd, file = paste("./Mapping.single.sp/Mantel.0.05/sig.sa.grnd.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
...
# The minimum number of occurences per transect will be determined by the variogram 

# For development purposes - lets start with a high number of occurences/transect so that we can get some nice clean variograms- 
# later, after everything is running smoothly, we can reduce the number of occurences/transect and watch the variograms deteriorate 

# Minimum number of occurences/transect = 31 = QQQ
# This results in 3 ground and 3 understory OTUs 
# you can run the above code - or save yourself some time and and just read it from a file 
...
#for (QQQ in 5:5) { # use this loop start to generate a series of tables (see line ~ 632: write.table(sig.results.undr, file = paste("./Mapping.single.sp/Mantel.0.05/sig.sa.undr.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
  ...
  # QQQ=2
  for (QQQ in 2:25) {  # this is the minimum number of times an OTU needs to be found in each transect - for a table use:             for (QQQ in 1:36) {
  ...
  write.table(gen.results, file = paste("./Mapping.single.sp/Mantel.0.05/Patch.occupancy/gen.grnd.undr.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
...
write.table(sig.results.undr, file = paste("./Mapping.single.sp/Mantel.0.05/sig.sa.gen.undr.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
write.table(sig.results.grnd, file = paste("./Mapping.single.sp/Mantel.0.05/sig.sa.gen.grnd.",QQQ,".occur.results.csv"), sep = ",", quote = FALSE, row.names = F)
